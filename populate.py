    ...
    for name in os.listdir('data'):
        ...
        if year:
            with open(os.path.join('data', name), encoding='cp1251') as csvfile:
                ...
                cursor.execute('SELECT record_id, is_done FROM zno_temp WHERE zno_year = %s', year)
                res = cursor.fetchone()

                if res is None:
                    cursor.execute('INSERT INTO zno_temp (zno_year, record_id, is_done) VALUES (%s, %s, %s)', year + [count, False])
                else:
                    if res[-1]:
                        continue
                ...
                for row in csv_reader:
                    ...
                    if not count % part_size:
                        psycopg2.extras.execute_values(cursor, sql_query[0], part)
                        cursor.execute(sql_query[1], [count, False] + year)
                        part = list()
                        connection.commit()
                if part:
                    psycopg2.extras.execute_values(cursor, sql_query[0], part)
                    cursor.execute(sql_query[1], [count, True] + year)
                    part = list()
                    connection.commit()
                    
                end = time.time() - start
                result.write(f'File {name} done. Execution time - {end} sec.\n')
